...
NOTE: 對於SAP HANA系統複寫、所取得的Snapshot複本總數將等於資源群組的保留集。  移除最舊的Snapshot複本是根據最舊Snapshot複本所在的節點而決定。
例如、對於使用SAP HANA System Replication一線資源群組和SAP HANA System Replication二線資源群組、保留值設為7。  您一次最多可取得7個Snapshot複本、包括SAP HANA System Replication一線複本和SAP HANA System Replication二線複本。

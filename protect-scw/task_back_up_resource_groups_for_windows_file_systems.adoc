...
+
您可以在搜尋方塊中輸入資源群組名稱、或按一下並選取標記、以搜尋資源群組 image:../media/filter_icon.gif["篩選圖示"] 。然後按一下 image:../media/filter_icon.gif["篩選圖示"][] 關閉篩選窗格。
...
** 如果您在VMDK上備份應用程式資料、SnapCenter 而針對VMware vSphere的VMware vSphere的Java堆疊大小不夠大、則備份可能會失敗。若要增加Java堆大小、請找出指令碼檔案 `/opt/netapp/init_scripts/scvservice`。在該指令碼中 `do_start method` 命令會啟動SnapCenter VMware的VMware插件服務。將該命令更新為： `Java -jar -Xmx8192M -Xms4096M`。
